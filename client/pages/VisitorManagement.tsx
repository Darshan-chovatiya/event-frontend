...
const VisitorManagement: React.FC = () => {
  ...
  const fetchVisitors = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BaseUrl}/admin/get-visitor-details`, {
        method: "POST",
        headers: {
      ...
      const data = await response.json();
      const mappedVisitors: Visitor[] = data.data.docs
        .filter((visitor: any) => !visitor.isDeleted)
        .map((visitor: any) => ({
          _id: visitor._id,
          name: visitor.name || "Unnamed",
          email: visitor.email,
          companyName: visitor.companyName || "N/A",
          mobile: visitor.mobile || "N/A",
          status: visitor.status,
          profileImage: visitor.profileImage || undefined,
          createdAt: visitor.createdAt ? new Date(visitor.createdAt).toISOString() : "",
  ...
  const handleDeleteVisitor = async (visitorId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${BaseUrl}/admin/delete-visitor`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      const data = await response.json();
      toast({ title: "Success", description: data.message || "Visitor deleted successfully" });
      fetchVisitors();
    } catch (err: any) {
  ...
  const handleStatusUpdate = async (visitorId: string, status: "active" | "inactive") => {
    setLoading(true);
    setError("");
    try {
      const token = localStorage.getItem("adminToken");
      ...
      const response = await fetch(`${BaseUrl}/admin/update-visitor-status`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({ id: visitorId, status }),
      });
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center ring-2 ring-blue-500/20 group-hover:ring-blue-500/40 transition-all duration-200 flex-shrink-0">
                            {visitor.profileImage ? (
                              <img
                                src={`${AmazonAws}/${visitor.profileImage}`}
                                alt={visitor.name}
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value: "active" | "inactive") =>
                              handleStatusUpdate(visitor._id, value)
                            }
                            disabled={loading}
                          >
                            <SelectTrigger
